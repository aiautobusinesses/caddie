-- Atomic Postgres functions for multi-step workflows.
-- All functions are in the public schema and callable via supabase.rpc().
-- SECURITY INVOKER is used for functions that operate on the calling user's data;
-- SECURITY DEFINER is used only for accept_invite which must bypass RLS on invites.
...
-- 2. Mark the live step done, advance live_step_id, record a step_event.
...
begin
  ...
  if v_thing.live_step_id is not null then
    ...
    from steps
    where thing_id = p_thing_id and done = false and id != v_thing.live_step_id
...
begin
  ...
  select id into v_next_step_id
  from steps
  where thing_id = v_step.thing_id and done = false and id != p_step_id
  order by step_order asc
  limit 1;

  update things set live_step_id = v_next_step_id
...
-- 4. Prepend a "Look up how to…" step before the current live step.
...
-- 5. Insert an entity and its care plan atomically.
...
-- 6. Accept an invite and upgrade the profile tier atomically.
--    Uses SECURITY DEFINER so it can bypass the invitees-can-read-own-invite RLS policy
--    and acquire a row-level lock (FOR UPDATE) to prevent double-acceptance.
...
-- 7. Batch-process a care-group report: update done plans, insert care_events
--    for all plans, and record the offer date — all in one transaction.
...
begin
  ...
  loop
    if v_plan.id = any(p_done_ids) then
      -- Compute next_due_at from intervals
